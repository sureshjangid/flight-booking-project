...
class RoundTrip extends Component{
    ...
    onDepartDate=(startDate)=>{
        const{onRoundWayDateChange}=this.props
        onRoundWayDateChange("deaprtDate",startDate);
        this.onReturnDate(startDate)
    }
    onReturnDate=(startDate)=>{
        const{onRoundWayDateChange}=this.props
        onRoundWayDateChange("returnDate",startDate);
        
    }

    onSearchFrom=(value)=>{
        console.log("onSearchFrom", value)
        this.props.RoundTripSearchUpdate("from",value)
    }
    onSearchTo=(value)=>{
        console.log("onSearchTo", value)
        this.props.RoundTripSearchUpdate("to",value)
    }
    isEmpty = (obj) => {
        return Object.keys(obj).length === 0;
    }
    render(){
        const{roundTrips,onClickToReverseRoundTrip,onSearchFlight}=this.props;
        console.log("roundTrips",roundTrips)
        return <> <div className="form-row">
        <div className="serach-box align-items-center">
            <div className="serach-box-item d-flex w-100 align-items-center search-rount-trip">
                <div className="form-group type-input from-to">
                    {/* <input type="text" className="form-control" placeholder="From?" value={roundTrips.from} /> */}
                    <PlaceSearch value={roundTrips.from && Object.keys(roundTrips.from).length>0 && roundTrips.from.name?roundTrips.from.name:""} onSearchUpdate={this.onSearchFrom} placeholder="Where From"/>
                </div>
                <div className="revert-box">
                    <button type="button" className="revert-btn" onClick={onClickToReverseRoundTrip}>
                ...
                <PlaceSearch value={roundTrips.to && Object.keys(roundTrips.to).length>0 && roundTrips.to.name?roundTrips.to.name:""}  onSearchUpdate={this.onSearchTo} placeholder="Where To"/>
                    {/* <input type="text" className="form-control" placeholder="To?" value={roundTrips.to} /> */}
                </div>
            </div>
            ...
            <div className="serach-box-item d-flex w-100 align-items-center">
                <div className="form-group type-input">
                    <DateRange onDateChange={this.onDepartDate} key={"onDepartDate"}>
                        <input type="text" className="form-control" placeholder="dd/mm/yyyy" value={moment(roundTrips.deaprtDate).format("DD/MM/YYYY")} />
                    </DateRange>
